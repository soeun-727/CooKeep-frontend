    ...
    <div className="flex flex-col items-start gap-[36px] w-full">
          ...
          <div className="flex flex-col items-start gap-2 w-full">
            ...
            <div className="flex flex-wrap items-start gap-[5px]">
              {requiredIngredients.map((item, idx) => (
                <span
                  key={idx}
                  className="flex items-center justify-center h-[20px] px-[12px] rounded-full bg-(--color-gray-10) text-(--color-gray-50) text-[12px] leading-[16px] font-medium"
                >
                  {formatIngredient(item)}
                </span>
              ))}
      ...
      {substitutions.length > 0 && (
        <div className="flex flex-col items-start gap-[10px] self-stretch w-full">
          {/* 섹션 타이틀 */}
          <span className="self-stretch text-(--color-gray-50) typo-label">
            대체/생략 가능 재료
          </span>
                ...
                >
                  ...
                  <div className="flex flex-wrap items-start content-start gap-x-[6px] gap-y-[5px] self-stretch">
                    {items.map((item, itemIdx) => (
                      <div
                        key={itemIdx}
                        className="flex px-[12px] py-[2px] justify-center items-center gap-[8px] rounded-[100px] bg-(--color-gray-10)"
                      >
                        <span className="text-(--color-gray-50) text-center text-[12px] leading-[16px] font-medium whitespace-nowrap">
                          {formatIngredient(item)}
                        </span>
                      </div>
                    ))}
