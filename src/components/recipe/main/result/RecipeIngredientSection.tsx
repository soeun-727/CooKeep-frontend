    ...
    <div className="flex w-full flex-col items-start gap-[36px]">
        ...
        <div className="flex w-full flex-col items-start gap-2">
          ...
          <div className="flex flex-wrap items-start gap-[5px]">
            {selectedIngredients.map((item, idx) => (
              <span
                key={idx}
                className="bg-green-deep text-gray-0 flex h-[20px] items-center justify-center rounded-full px-[12px] text-[12px] leading-[16px] font-medium"
              >
          ...
          <div className="flex w-full flex-col items-start gap-2">
            ...
            <div className="flex flex-wrap items-start gap-[5px]">
              {requiredIngredients.map((item, idx) => (
                <span
                  key={idx}
                  className="bg-gray-10 flex h-[20px] items-center justify-center rounded-full px-[12px] text-[12px] leading-[16px] font-medium text-gray-50"
                >
                  {formatIngredient(item)}
                </span>
              ))}
      ...
      {substitutions.length > 0 && (
        <div className="flex w-full flex-col items-start gap-[10px] self-stretch">
          {/* 섹션 타이틀 */}
          <span className="typo-label self-stretch text-gray-50">
            대체/생략 가능 재료
          </span>
                ...
                >
                  ...
                  <div className="flex flex-wrap content-start items-start gap-x-[6px] gap-y-[5px] self-stretch">
                    {items.map((item, itemIdx) => (
                      <div
                        key={itemIdx}
                        className="bg-gray-10 flex items-center justify-center gap-[8px] rounded-[100px] px-[12px] py-[2px]"
                      >
                        <span className="text-center text-[12px] leading-[16px] font-medium whitespace-nowrap text-gray-50">
                          {formatIngredient(item)}
                        </span>
                      </div>
                    ))}
