...
export default function RecipeYoutubeCard({ videos, tags = [] }: Props) {
  ...
  return (
    <section className="mx-auto flex w-full flex-col gap-2 rounded-[6px] bg-white p-[22px_15px] shadow-[0_4px_16px_-10px_rgba(0,0,0,0.25)]">
      ...
      {tags.length > 0 && (
        <div className="flex flex-wrap gap-x-2 gap-y-[2px]">
          {tags.map((tag, idx) => (
            <span key={idx} className="typo-body2 text-[#32E389]">
              {`# ${tag}`}
            </span>
          ))}
        </div>
